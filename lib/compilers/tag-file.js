...
var extractTagInfo = function (lines) {
  ...
  let className = camelCase(tagName)
  return {tagName, className, tagAttributes}
}
...
module.exports.compile = function (content) {
  ...
  var result = `
  const WebComponent = require('organic-oval').WebComponent
  const TagClass = class ${tagInfo.className} extends WebComponent {
    constructor () {
      super()
      ${scriptContent.trim()}
      this.render = () => {
        return this.html\`${htmlContent.trim()}\`
      }
    }
  }
  TagClass.define('${tagInfo.tagName}')
