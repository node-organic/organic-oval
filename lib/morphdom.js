...
const setProps = function (el, props) {
  for (let key in props) {
    if (typeof props[key] === 'string' || typeof props[key] === 'number') {
      ...
      if (key === 'ref') {
        if (currentComponent.refs[props[key]] && currentComponent.refs[props[key]].attributes.freeze) {
          // do nothing because 
          // 1) ref exists
          // 2) the ref is freezed, ie it wont be morphed, 
          // thus the valid reference will be overriden with unused one
        } else {
          currentComponent.refs[props[key]] = el
        }
      }
    }
